...
def main(args):
    ...
    train_dset, train_loader = data_loader(args, train_path, phase='training', split='train')
    logger.info("Initializing val dataset")
    _, val_loader = data_loader(args, val_path, phase='training', split='val')

    iterations_per_epoch = len(train_dset) / args.batch_size / args.d_steps
    if args.num_epochs: args.num_iterations = int(iterations_per_epoch * args.num_epochs)
    ...
    generator = TrajectoryGenerator(obs_len=args.obs_len, pred_len=args.pred_len, embedding_dim=args.embedding_dim,
        ...
        noise_dim=args.noise_dim, noise_type=args.noise_type, noise_mix_type=args.noise_mix_type,
        pooling_type=args.pooling_type, pool_every_timestep=args.pool_every_timestep, dropout=args.dropout,
        bottleneck_dim=args.bottleneck_dim, neighborhood_size=args.neighborhood_size, grid_size=args.grid_size, batch_norm=args.batch_norm)
    ...
    discriminator = TrajectoryDiscriminator(obs_len=args.obs_len, pred_len=args.pred_len, embedding_dim=args.embedding_dim,
        h_dim=args.encoder_h_dim_d, mlp_dim=args.mlp_dim, num_layers=args.num_layers, dropout=args.dropout, batch_norm=args.batch_norm, d_type=args.d_type)
    ...
    if restore_path is not None and os.path.isfile(restore_path):
        ...
        t, epoch = 0, 0
        checkpoint = {'args': args.__dict__, 'G_losses': defaultdict(list), 'D_losses': defaultdict(list),
            'losses_ts': [], 'metrics_val': defaultdict(list), 'metrics_train': defaultdict(list),
            'sample_ts': [], 'restore_ts': [], 'norm_g': [], 'norm_d': [],
            'counters': {'t': None, 'epoch': None,},
            'g_state': None, 'g_optim_state': None, 'd_state': None, 'd_optim_state': None, 'g_best_state': None, 
            'd_best_state': None, 'best_t': None, 'g_best_nl_state': None, 'd_best_state_nl': None, 'best_t_nl': None,}
    t0 = None
    while t < args.num_iterations:
        ...
        for batch in train_loader:
            ...
            if d_steps_left > 0 or g_steps_left > 0: continue
            if args.timing == 1:
                if t0 is not None: logger.info('Interation {} took {}'.format(t - 1, time.time() - t0))
                t0 = time.time()
            ...
            if t > 0 and t % args.checkpoint_every == 0:
                ...
                if epoch % 50 == 0:
                    checkpoint_path = os.path.join(args.output_dir, '%s_with_model_epoch_%04d_%s.pt' % (args.checkpoint_name, epoch, args.dataset_name))
            ...
            d_steps_left = args.d_steps
            g_steps_left = args.g_steps
            if t >= args.num_iterations: break
...
def check_accuracy(args, loader, generator, discriminator, d_loss_fn, limit=False):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            ade, ade_l, ade_nl = cal_ade(pred_traj_gt, pred_traj_fake, linear_ped, non_linear_ped, loss_mask)
            fde, fde_l, fde_nl = cal_fde(pred_traj_gt, pred_traj_fake, linear_ped, non_linear_ped, loss_mask)
    ...
    if total_traj_l != 0:
        metrics['ade_l'] = sum(disp_error_l) / (total_traj_l)
