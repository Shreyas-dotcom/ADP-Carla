...
def main():
    ...
    try:
        ...
        KARlab_physics(vehicle, weight_KARlab, max_steer_angle, radius_front_wheels, radius_rear_wheels,
        ...
        spectator.set_transform(carla.Transform(transform.location + carla.Location(x=-4, y=-8, z=10),
                                                carla.Rotation(pitch=-40, yaw=110)))  # 3rd person view
        ...
        world.debug.draw_string(parking_location, 'O   PARKING TARGET', draw_shadow=False,
                                color=carla.Color(0, 255, 0, 0), life_time=200, persistent_lines=True)
        ...
        collision_sensor = collision_sensor_creation(world, actor_list, vehicle, bp_collision_sensor,
                                                     collision_sensor_offset)

        collision_sensor.listen(lambda collision_sensor_data: collision_detector(collision_sensor_data, collision_hist,
                                                                                 show_debug_collision_sensor))  # Collision sensor is listening
        ...
        obstacle_sensor = obstacle_sensor_creation(world, actor_list, vehicle, bp_obstacle_sensor,
                                                   obstacle_sensor_offset, distance=7, hit_radius=2.5)

        obstacle_sensor.listen(lambda obstacle_sensor_data: obstacle_detector(obstacle_sensor_data, obstacle_hist,
                                                                              show_debug_obstacle_sensor))  # Obstacle Sensor is listening
        ...
        radar_sensor = radar_creation(world, actor_list, vehicle, bp_radar_sensor, radar_offset, horizontal_fov=120,
                                      vertical_fov=0.001, range=80, points_per_second=100, sensor_tick=0.1)
        ...
        if (show_bounding_box):
            draw_vehicle_collider(world, col_size, box_collider.location.z,
                                  carla.Color(255, 0, 0, 0))  # COLLIDERS Vehicule and KARlab
...
def normalize_radar(radar_vect):
    ...
    """
    length = math.sqrt(
        radar_vect.velocity ** 2 + radar_vect.azimuth ** 2 + radar_vect.altitude ** 2 + radar_vect.depth ** 2)  # !!! Maybe we have to divide y the max value i.e. the length of the map !!!
    radar_tab = [radar_vect.velocity, radar_vect.azimuth, radar_vect.altitude, radar_vect.depth]
    res_radar_norm = [x / length for x in
                      radar_tab]  # divide all the element of the tab by the norm of the initial vector
...
def collision_sensor_creation(world, actor_list, vehicle, bp_collision_sensor, collision_sensor_offset):
    ...
    collision_sensor = world.spawn_actor(bp_collision_sensor, carla.Transform(collision_sensor_offset),
                                         attach_to=vehicle)
    actor_list.append(collision_sensor)
    print(('Created %s' % collision_sensor.type_id) + ' at position: ' + str(
        collision_sensor_offset) + ' from the vehicle')
...
# OBSTACLES SENSOR
def obstacle_sensor_creation(world, actor_list, vehicle, bp_obstacle_sensor, obstacle_sensor_offset, distance,
                             hit_radius):
    ...
    print(
        ('Created %s' % obstacle_sensor.type_id) + ' at position: ' + str(obstacle_sensor_offset) + ' from the vehicle')
...
def obstacle_detector(event, obstacle_hist, show_debug_obstacle_sensor):
    ...
    if (show_debug_obstacle_sensor): print('\n!!! Obstacle ' + str(actor_we_detect) + ' at a distance of ' + str(
        round(distance_actor_we_detect, 2)) + 'm detected !!!\n')
...
def rad_callback(world, radar_data, show_debug_radar):
    ...
    for detect in radar_data:
        ...
        if (show_debug_radar): world.debug.draw_point(radar_data.transform.location + fw_vec, size=0.075,
                                                      life_time=0.06, persistent_lines=False,
        ...
        if (show_debug_radar): print('- n°' + str(index) + ' ' + str(detect) + '\nDetection_norm:' + str(
            normalize_radar(detect)) + ' ; ID = [frame:' + str(radar_data.frame) + ' | timestamp: ' + str(
            radar_data.timestamp) + ' | points:' + str(
